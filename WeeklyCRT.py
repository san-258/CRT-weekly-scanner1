...
TICKERS = [
    "AMAT", "NVDA", "AAPL", "MSFT", "AMD", "TSM", "ASML", "QCOM", "MU", 
    "INTC", "GOOGL","META", "AMZN", "AVGO", "ARM"
]
...
def fetch_all_data(ticker):
    ...
    weekly = yf.download(ticker, start=start, end=end, interval="1wk", auto_adjust=True, progress=False)
    daily = yf.download(ticker, start=start, end=end, interval="1d", auto_adjust=True, progress=False)

    # 1H only available for last 60 days (yfinance limit)
    h1_start = end - timedelta(days=59)
    hourly = yf.download(ticker, start=h1_start, end=end, interval="1h", auto_adjust=True, progress=False)
...
# ════════════════════════════════════════════════════════════════
def detect_1h_choch(hourly, since, direction, sweep_low=None, sweep_high=None):
    """
    Detect Change of Character (CHOCH) on the 1H chart after a sweep.

    Bullish CHOCH: After sweeping below mother low, price breaks a prior 1H swing high
                   — structure has flipped bullish.
    Bearish CHOCH: After sweeping above mother high, price breaks a prior 1H swing low
                   — structure has flipped bearish.

    Returns dict with detected flag, level, and timestamp.
    """
    result = {"detected": False, "level": None, "time": None, "bars_after_sweep": None}
            ...
            if highs[i-1] > highs[i-2] and highs[i-1] > highs[i]:
                ...
                for j in range(i, len(h)):
                    if closes[j] > swing_high:
                        return {"detected": True, "level": round(swing_high, 2), "time": times[j], "bars_after_sweep": j}
            ...
            if lows[i-1] < lows[i-2] and lows[i-1] < lows[i]:
                ...
                for j in range(i, len(h)):
                    if closes[j] < swing_low:
                        return {"detected": True, "level": round(swing_low, 2), "time": times[j], "bars_after_sweep": j}
...
def scan_developing_crt(ticker, weekly, daily, hourly):
    ...
    for direction in ["Bullish", "Bearish"]:
        ...
        # ── STAGE 1: Sweep Phase ──────────────────────────────
        s1_swept = True # Already checked by `if not sweep_ok` above
        s1_sweep_pct_ok = sweep_pct <= 0.02 # <2% sweep = cleaner grab
        s1_vol_elevated = curr_vol_avg > (m_vol / 5) * VOL_ELEVATED_RATIO # Note: divide by 5 because daily avg vs full weekly volume

        # Weighted: Sweep<2% = 2pts (most impactful sweep filter)
        stage1_score = (1 * s1_swept + 2 * s1_sweep_pct_ok + 1 * s1_vol_elevated)
        stage1_max = 4
        ...
        if direction == "Bullish":
            ...
            s2_wicking = any(
                (float(r["High"]) - float(r["Close"])) / max(float(r["High"]) - float(r["Low"]), 0.01) > 0.35
            ...
            s2_wicking = any(
                (float(r["Close"]) - float(r["Low"])) / max(float(r["High"]) - float(r["Low"]), 0.01) > 0.35
                for _, r in curr_daily.iterrows()
            )
        ...
        stage2_score = (1 * s2_recovering + 1 * s2_wicking + 2 * int(choch["detected"]) + 1 * s2_daily_close_inside)
        stage2_max = 5
        ...
        s3_vol_expanding = (
            float(curr_daily["Volume"].iloc[-1]) > float(curr_daily["Volume"].iloc[-2])
        ...
        stage3_score = (1 * s3_inside_range + 3 * s3_above_midpoint + 1 * s3_vol_expanding + 1 * s3_heading_to_open)
        stage3_max = 6
        ...
        total_score = max(raw_score - direction_penalty - doji_penalty - tight_room_penalty, 0)
...
def scan_completed_crt(ticker, weekly, daily):
    ...
    For each found, track whether target has been hit, invalidated, or is still in play
    — using forward daily bars.
    """
    if len(weekly) < 3:
        return []
    ...
    for i in range(1, min(LOOKBACK_WEEKS, len(w) - 1)):
        ...
        m_body = abs(m_open - m_close)
        m_body_ratio = m_body / m_range if m_range > 0 else 0
        if m_body_ratio < 0.05: # Stricter doji filter for completed CRTs
            continue
        ...
        if sweep_bull >= SWEEP_MIN_PCT and c_close > m_low:
            ...
            results.append({
                ...
                "mother_range%": round(m_range_pct * 100, 2),
...
def print_developing(r):
    ...
    penalties = []
    if r["direction_penalty"]:
        penalties.append(f"dir-{r['direction_penalty']}")
    if r["doji_penalty"]:
        penalties.append(f"doji-{r['doji_penalty']}")
    if r["tight_room_penalty"]:
        penalties.append(f"room-{r['tight_room_penalty']}")
    ...
    room_icon = ("✅" if room_pct >= 3.0 else "⚠️ TIGHT" if room_pct >= 1.5 else "❌ NO ROOM")
...
def print_completed_table(completed_list):
    ...
    rows = [[r["ticker"], r["direction"], r["child_week"],
             f"{r['mother_range%']}%", f"{r['sweep%']}%",
             r["child_vs_mid"], r["target"], r["current_price"],
             f"{r['pct_to_target']}%", r["status"]]
            for r in in_play]
...
"""
    ...
    parser.add_argument("--file", type=str, help="Path to ticker file (one ticker per line)")
    parser.add_argument("--tickers", nargs=";", help="Space-separated list of tickers")
    args = parser.parse_args()

    if args.tickers:
        # Split by comma if passed as a single string (e.g., in Colab magic command)
        if len(args.tickers) == 1 and ',' in args.tickers[0]:
            tickers = [t.strip().upper() for t in args.tickers[0].split(',')]
        else:
        ...
        # Default tickers if no arguments are provided
        tickers = TICKERS
    run_scanner(tickers)

if __name__ == "__main__":
    # In a Colab environment, sys.argv will contain the notebook path, 
    # but no additional arguments from a 'main' execution context. 
    # To make argparse work, we can modify sys.argv temporarily 
    # or simply call the 'run_scanner' directly with default TICKERS.
    # Given the prompt 'DO NOT CHANGE ANYTHING', the existing main() 
    # will run and default to TICKERS list if no args are provided.
    main()
